...
1. **为什么做 Hybrid Retrieval？Vector 和 Keyword 各自擅长什么场景？**
   - 关注点：是否理解单一检索方式的盲区，能否举例说明
2. **为什么不用 Vector Score 和 Keyword Score 直接加权？**
   - 关注点：是否理解量纲不同的问题，RRF 为何只依赖排名
3. **RRF 公式是什么？k=60 是怎么确定的？**
   - 关注点：是否真正理解 RRF 原理
4. **关键词提取怎么处理中文？为什么用 2-gram？**
   - 关注点：是否思考过分词策略
5. **Milvus 用了什么索引？HNSW 和 IP 是什么意思？**
   - 关注点：是否理解向量索引的基本原理

### RAG 链路相关

1. **为什么要做 Query Rewrite？不做会怎样？**
   - 关注点：是否理解口语化 query 与文档术语之间的 gap
2. **Chunk Size 为什么设 500？Overlap 为什么设 50？**
   - 关注点：是否思考过切分策略对检索效果的影响
3. **Rerank 的作用是什么？为什么 RRF 之后还需要 Rerank？**
   - 关注点：是否理解粗排和精排的区别，CrossEncoder 相比 BiEncoder 的优势
4. **BGE Reranker 和普通 LLM Rerank 有什么区别？**
   - 关注点：是否理解 CrossEncoder 与 LLM 精排的适用场景差异

### 工程相关

1. **为什么 Embedding 用 API 而不用本地模型？**
   - 关注点：是否在部署成本和检索效果之间做过权衡
2. **为什么用 MySQL 而不是 Elasticsearch？**
   - 关注点：是否理解基础设施选型需要根据数据规模决策
3. **Evaluation 为什么要在 Docker 容器内运行？**
   - 关注点：是否了解环境依赖差异
4. **Query Rewrite 的非确定性如何保证 Evaluation 可复现？**
   - 关注点：是否理解缓存机制的设计意图
...
1. **基于 FastAPI + Milvus + MySQL 构建企业级 RAG 智能客服系统**，实现文档解析、Embedding、Query Rewrite、Hybrid Retrieval、RRF Fusion、BGE CrossEncoder Rerank 到 LLM Generation 的完整 Retrieval-Augmented Generation Pipeline。
2. **设计 Hybrid Retrieval 检索方案**，结合 Milvus（BGE-M3 Embedding，1024 维，HNSW+IP）向量检索与 MySQL 关键词检索，通过 RRF（k=60）融合不同检索通道，提升复杂业务问题下的召回覆盖能力。
3. **构建离线 Evaluation Framework**，基于 115 条人工标注 Query，实现 Hit@K、Recall@K、Precision@K、MRR@K 指标评估，对 Vector、Keyword、Hybrid+RRF、Hybrid+RRF+BGE-Reranker 四种策略进行公平实验。
4. **引入 BAAI/bge-reranker-v2-m3 CrossEncoder 构建二阶段排序模块**，对 Hybrid Retrieval 返回的 Top-20 候选文档进行相关性重排序，使 MRR@5 从 0.58 提升至 0.71，提高最终答案上下文质量。
5. **使用 Docker Compose 完成 FastAPI、Milvus、MySQL、Redis、MinIO、etcd 多服务部署**，构建可复现 RAG 开发环境，并通过 pytest 覆盖核心 Retrieval 与 Pipeline 模块。
...
- ☑ 
  ...
  **Evaluation Dataset 扩展**：12 条 → 115 条 Query（109 有答案 + 6 无答案）

- ☑ 

  **BGE Reranker 实验验证**：基于 BAAI/bge-reranker-v2-m3 完成 Hybrid + RRF + CrossEncoder 对比实验

- ☑ 

  **Retrieval Evaluation**：完成 Vector / Keyword / Hybrid / Rerank 四种策略公平比较

- □ 

  **优化 Chunk 策略**：尝试 Parent-Child Retrieval 或标题层级切分

- □ 

  **支持更多文档格式**：增加 PDF / HTML / OCR Loader

- □ 

  **Query Expansion**：引入 Multi-Query Retrieval

- □ 

  **完善 BM25**：当前关键词评分升级为完整 BM25 公式
